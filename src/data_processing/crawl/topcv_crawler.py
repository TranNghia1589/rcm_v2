...
def build_fields_config(session: requests.Session):
    ...
    fields = [
        ...
        {"slug": "software-engineer", "name_vn": "Software Engineer"},
        {"slug": "backend-developer", "name_vn": "Backend Developer"},
        {"slug": "frontend-developer", "name_vn": "Frontend Developer"},
        {"slug": "fullstack-developer", "name_vn": "Fullstack Developer"},
        {"slug": "cloud-engineer", "name_vn": "Cloud Engineer"},
        {"slug": "database-administrator-dba", "name_vn": "Database Administrator (DBA)"},
        {"slug": "automation-tester", "name_vn": "Automation Tester"},
        {"slug": "devops-engineer", "name_vn": "DevOps Engineer"},
        {"slug": "ky-su-iot-iot-engineer", "name_vn": "Kỹ sư IoT (IoT Engineer)"},
        {"slug": "embedded-engineer-lap-trinh-nhung", "name_vn": "Embedded Engineer/Lập trình nhúng"},
        {"slug": "product-analyst-research", "name_vn": "Product Analyst/Research"},
        {"slug": "business-analyst-phan-tich-nghiep-vu", "name_vn": "Business Analyst (Phân tích nghiệp vụ)"},
        {"slug": "product-owner-product-manager", "name_vn": "Product Owner/Product Manager"},
    ]
...
def main(output_dir: str | Path | None = None) -> None:
    ...
    fields = build_fields_config(session)

    # Resolve project root from src/data_processing/crawl/topcv_crawler.py
    base_dir = Path(__file__).resolve().parents[3]
    resolved_output_dir = Path(output_dir) if output_dir else (base_dir / "data" / "raw" / "jobs")
    ...
    if not all_dfs:
        print("[WARN] No crawl data collected.")
        return

    print(f"\n{'=' * 60}")
    print(" PREPARE DATA BEFORE EXPORT ")
    print(f"{'=' * 60}")

    merged_df = merge_jobs_by_url(all_dfs)
    if merged_df.empty:
        print("[WARN] No valid merged records to save.")
        return

    print(f"\n{'=' * 60}")
    print(" SAVE OUTPUT FILES ")
    print(f"{'=' * 60}")
    save_output_files(merged_df, str(resolved_output_dir), base_name)
    print("[DONE] TopCV crawl completed.")
