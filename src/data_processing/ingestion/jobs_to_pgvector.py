...
from src.utils.infrastructure.db.pgvector_store import PgVectorStore
from src.utils.infrastructure.db.postgres_client import PostgresClient, PostgresConfig
from src.models.rag.chunking import ChunkingConfig, split_text
from src.models.rag.embed import create_embedder_from_yaml
from src.models.rag.index import index_job_documents
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--jobs_path",
        default=str(BASE_DIR / "experiments" / "artifacts" / "matching" / "jobs_chatbot_sections.parquet"),
    ...
    parser.add_argument(
        "--pgvector_config",
        default=str(BASE_DIR / "config" / "db" / "pgvector.yaml"),
        help="Path to pgvector yaml config.",
    )
    parser.add_argument(
        "--chunking_config",
        default=str(BASE_DIR / "config" / "rag" / "chunking.yaml"),
